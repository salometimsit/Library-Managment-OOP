...
class BooksFactory:
    ...
    def add_to_available(self,copies,book):
        title=book.get_title()
        author=book.get_author()
        genre=book.get_genre()
        year=int(book.get_year())
        ...
        book_found = not_available_df[(not_available_df['author'].str.lower() == author.lower()) &
                                      ...
                                      (not_available_df['year'].astype(int) == year)]
        ...
        if not book_found.empty and 'waiting_list' in book_found.columns:
            ...
            if pd.notna(waiting_list) and waiting_list.strip():
                ...
                for person in waiting_people:
                    ...
                    lib.notify(f"The Book '{title}' has been returned and should be transferred to {name},\n"
                   f" Phone: {phone} \n Email: {email}")
                    lib.rent_book(book)
        ...
        if copies > 0:
            ...
            available_book = available_df[(available_df['author'].str.lower() == author.lower()) &
                                          ...
                                          (available_df['year'].astype(int) == year)]

            if not available_book.empty:
                available_df.loc[(available_df['author'].str.lower() == author.lower()) &
                                 (available_df['title'].str.lower() == title.lower()) &
                                 (available_df['genre'].str.lower() == genre.lower()) &
                                 (available_df['year'].astype(int) == year), "copies"] += copies
            else:
                new_line = {"title": title,"author": author,"is_loaned": "No","copies": copies,"genre": genre,
                    "year": year,"popularity": book.get_popularity()}
                available_df = pd.concat([available_df, pd.DataFrame([new_line])], ignore_index=True)

            available_df.to_csv(self.__files[1], index=False)

    def check_not_available(self,book):
        title=book.get_title().lower()
        genre=book.get_genre().lower()
        author=book.get_author().lower()
        year=int(book.get_year())

        not_available_df = pd.read_csv(self.__files[2])
        filtered_non_available_df = not_available_df[(not_available_df['author'].str.lower() == author) &
                                             (not_available_df['title'].str.lower() == title) &
                                             (not_available_df['genre'].str.lower() == genre) &
                                             (not_available_df['year'].astype(int) == year)]
        if not filtered_non_available_df.empty:
            not_available_df.loc[(not_available_df['author'].str.lower() == author) &
                                 (not_available_df['title'].str.lower() == title)&
                                 (not_available_df['genre'].str.lower() == genre) & (not_available_df['year'] == year),
            "is_loaned"] ="No"
        not_available_df.to_csv(self.__files[2], index=False)
    ...
    def create_books(self,title, author, copies,genre,year):
        ...
        self.add_to_available(copies,book)
